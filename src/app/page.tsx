...
function SettingsView() {
  ...
  useEffect(() => {
    ...
    if (!hash) return
    if (hash === 'chats') {
      window.dispatchEvent(new CustomEvent('aside:view', { detail: 'chats' }))
      return
    }
    const [p, mode] = hash.split('-')
    if (p) setPage(p)
...
function Shell() {
  ...
  useEffect(() => {
    if (window.location.hash.replace(/^#\/?/, '') === 'chats') setView('chats')
    const onView = (e: Event) => setView((e as CustomEvent).detail)
    window.addEventListener('aside:view', onView)
    return () => window.removeEventListener('aside:view', onView)
  }, [])
  return (
